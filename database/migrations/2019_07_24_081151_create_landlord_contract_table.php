...
class CreateLandlordContractTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('landlord_contract', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('commissioner_id')->nullable()->comment('專員 ID');
            $table->string('commission_type')->comment('承租方式');
            $table->date('commission_start_date')->nullable()->comment('委託起');
            $table->date('commission_end_date')->nullable()->comment('委託迄');
            $table->date('warranty_start_date')->nullable()->comment('保固起');
            $table->date('warranty_end_date')->nullable()->comment('保固迄');
            $table->date('rental_decoration_free_start_date')->nullable()->comment('免租金裝潢起');
            $table->date('rental_decoration_free_end_date')->nullable()->comment('免租金裝潢迄');
            ...
            $table->date('rent_adjusted_date')->nullable()->comment('租金調整日');
