...
class PeriodService{
    ...
    public function every(string $period, $from, $to, $callback) {
        if (isset($this->monthsOf[$period])) {
            return $this->getPeriods($from, $to, $this->monthsOf[$period])->map($callback);
        } else {
    ...
    public function next($date, string $period) {
        return Carbon::parse($date)->copy()->addMonthsWithoutOverflow($this->monthsOf[$period]);
    }
    ...
    public function getPeriods($from, $to, int $months) {
        return collect(
                    Carbon::parse($from)->range($to, $months, 'month')->excludeEndDate()
                )->map(function ($date, $i) use ($from, $months) {
                    return Carbon::parse($from)->copy()->addMonthsWithoutOverflow($months * $i);
                });
    }
}