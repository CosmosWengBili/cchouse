...
class TenantContractService{
    ...
    public function create($data, $payments = []) {
        ...
        foreach ($payments as $payment) {
            ...
            $futurePayments = $this->periodService->every($payment['period'], $tenantContract->contract_start, $tenantContract->contract_end, function ($date) use ($payment) {
    ...
    public function makeExtendedContract(TenantContract $oldTenantContract, $months = 1) {
        ...
        $newTenantContract->contract_end = $newStart->copy()->addMonthsWithoutOverflow($months)->format('Y-m-d');
